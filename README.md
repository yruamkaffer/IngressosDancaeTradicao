...
> `GATE_API_KEY` is the shared secret used by the turnstile/gate device to call the online validation API.
...
## 🚪 Online Gate / Turnstile API

The event gate should validate tickets online against the application before allowing entrance.

Recommended flow:

1. The turnstile reads the ticket QR Code.
2. It sends either the extracted `ticketCode` or the raw QR payload to the API.
3. The API validates the ticket in Supabase.
4. If the ticket is paid and unused, the API marks it as used and returns `allowed: true`.
5. If the ticket is invalid, unpaid or already used, the API returns `allowed: false`.
...
```json
{
  "ticketCode": "TCK-ABC1234567"
}
```

Request with raw QR payload:
...
```json
{
  ...
  "data": {
    "allowed": false,
    "code": "ALREADY_USED",
    "reason": "Ticket já utilizado."
  }
}
```

The gate device should open only when `ok` is `true` and `data.allowed` is `true`.
...
6. Configure `GATE_API_KEY` in Vercel and in the turnstile/gate integration.
7. Replace the placeholder Pix QR Code images with the real Pix QR Codes:
...
- The online gate API requires `GATE_API_KEY` and marks each ticket as used on the first valid scan.
